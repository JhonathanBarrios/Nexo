...
interface PaymentModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (amount: number, payAllPending: boolean) => void;
  payment: {
    description: string;
    amount: number | null;
...
export default function PaymentModal({
  ...
  const currentAmount = payAllPending ? totalPending : parseFloat(amount || '0');
  ...
  }, [payment, totalPending]);

  if (!payment) return null;

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onConfirm(currentAmount, payAllPending);
  };
